...
public class AuthenticationTokenDao {
    ...
    public void deleteOldSessionToken(String userId) {
        ...
        q.setParameter(ConstantsDao.userId, userId);
        q.setParameter(ConstantsDao.longLasted, false);
        q.setParameter(ConstantsDao.minDate, DateTime.now().minusDays(1).toDate());
        q.executeUpdate();
    }
    ...
    public void updateLastConnectionDate(String id) {
        ...
        EntityManager em = ThreadLocalContext.get().getEntityManager();
        Query q = em.createNativeQuery(sb.toString());
        q.setParameter(ConstantsDao.currentDate, new Date());
        q.setParameter(ConstantsDao.id, id);
        q.executeUpdate();
    }
